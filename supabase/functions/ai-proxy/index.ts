...
// Model routing.
//
// Clients may send `model_hint: 'fast' | 'smart'`. It is a HINT resolved
// against this whitelist — never a raw model name, so a tampered client
// cannot select an expensive model or an unvetted one. Unset/unknown = smart,
// which is exactly the pre-hint behavior for builds 160/161 in the field.
//
// Why 'fast' exists: meal-plan generation on sonnet took 40-55s per attempt
// (3/3 timeouts against the iOS ~60s fetch ceiling, confirmed in logs
// Aug 20). Haiku generates the same structured JSON several times faster;
// the client's validator (validateMealPlan) remains the quality gate either
// way — a plan that fails validation retries with a correction, so the model
// swap changes latency, not the acceptance bar.
// ---------------------------------------------------------------------------
const MODEL_WHITELIST: Record<string, string> = {
  fast: 'claude-haiku-4-5',
  smart: 'claude-sonnet-4-6',
}
const DEFAULT_MODEL = MODEL_WHITELIST.smart

// Hard server-side ceilings. max_tokens arrived client-supplied and was
// forwarded verbatim (audit P1: a tampered client could request 64k-token
// outputs on our key). 8192 matches the largest legitimate use (7-day meal
// plan). Text-input cap blocks stuffed-context abuse without touching any
// legitimate call: the biggest real prompt (coach context + 15 turns) is
// well under 60k chars.
...
// Pull the first JSON array out of a model reply. Haiku at max_tokens 300
// truncated its own JSON mid-array ("memory extraction failed: SyntaxError"
// live in logs) — the cap is now 600 AND the parse tolerates surrounding
// prose/fences without ever accepting a truncated array.
...
// Total characters of TEXT content across all messages (images not counted —
// they have their own economics and legitimate photos are large).
function textChars(messages: any[]): number {
  let n = 0
  for (const m of messages ?? []) {
    if (typeof m?.content === 'string') n += m.content.length
    else if (Array.isArray(m?.content)) {
      for (const b of m.content) if (b?.type === 'text' && typeof b.text === 'string') n += b.text.length
    }
  }
  return n
}

// One call to Anthropic with a single retry on 429/529/overloaded. The old
// path re-labeled EVERY Anthropic error as HTTP 400 with no Retry-After, so
// a burst of meal plans produced "Could not generate" alerts and synchronized
// retaps (audit finding). Real status + Retry-After now pass through.
async function callAnthropic(body: string): Promise<Response> {
  ...
  if (res.status === 429 || res.status === 529 || res.status === 500) {
    ...
    console.log(`Anthropic ${res.status}, retrying once in ${waitMs}ms`)
...
serve(async (req) => {
  ...
  if (url.pathname.endsWith('/food-search')) {
    ...
    if (!res.ok) {
      // Silent {foods: []} on upstream failure looked identical to "no
      // results" — the client showed an empty list instead of an error.
      console.error('USDA upstream error:', res.status)
      return errorResponse(res.status === 429 ? 429 : 502, 'Food database is temporarily unavailable - try again shortly')
    }
    const data = await res.json()
  ...
  try {
    ...
    const { messages, system, max_tokens, model_hint } = await req.json()

    if (textChars(messages) > MAX_TEXT_CHARS) {
      return errorResponse(413, 'Request too large')
    }
    const cappedMaxTokens = Math.min(Number(max_tokens) || 8192, MAX_TOKENS_CEILING)
    const model = MODEL_WHITELIST[model_hint as string] ?? DEFAULT_MODEL
    ...
    if (!text) {
      ...
      if (response.status === 429 || response.status === 529) {
        // Pass the REAL status through. The client can now distinguish
        // "slow down" from "your request was bad".
        const retryAfter = response.headers.get('retry-after') || '15'
        await response.body?.cancel()
        return errorResponse(response.status, 'The AI service is busy - please try again in a moment', { 'Retry-After': retryAfter })
      }

      const data = await response.json()
      console.log(`Anthropic response (${model}, ${Date.now() - t0}ms):`, JSON.stringify(data).substring(0, 300));
      if (data.type === 'error') {
