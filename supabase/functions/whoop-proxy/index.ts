  ...
  try {
    switch (body.action) {
      ...
      case 'summary': {
        ...
        // A healthy token whose account has NEVER recorded anything (all three
        // collections empty with no date filter) almost always means the OAuth
        // sign-in went into a different Whoop account than the strap's — found
        // live: sign-in email went to one address, strap lived on another.
        // Surface it so the app can say "wrong account?" instead of "wear your
        // strap", which sends the user to fix the one thing that isn't broken.
        const emptyAccount =
          (recData?.records?.length ?? 0) === 0 &&
          (cycleData?.records?.length ?? 0) === 0 &&
          (sleepData?.records?.length ?? 0) === 0

        return okResponse({ recovery, strain, sleep, recoveryHistory, sleepHistory, emptyAccount })
      }
